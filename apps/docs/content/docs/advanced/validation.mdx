...
|----------|-----------------|--------|
| `createMarzbanSDK(config)` | Config object fields and types | internal — not exported; validated via `validateConfig()`, which throws `ConfigurationError` |
...
try {
  ...
  if (isConfigurationError(err)) {
    ...
    console.error(err.details)  // Array of Zod issues (ZodError.issues), e.g. [{ path: ['baseUrl'], message: '...' }]
  }
}
```
