---
title: NestJS
description: Integrate MarzbanSDK into NestJS by wrapping the client in a global module that initializes it on onModuleInit and exposes it as an injectable service.
---
...
The `webhook.secret` passed to `createMarzbanSDK` above enables HMAC-SHA256 verification of incoming Marzban webhook payloads: `sdk.webhook.handleWebhook(rawBody, signature)` checks the signature, then emits each event to any listener registered with `sdk.webhook.on(action, handler)` (typed `WebhookType` payloads — `user_created`, `user_expired`, `reached_usage_percent`, and so on). For a full NestJS controller wiring up a raw-body route and registering those listeners, see the [Webhooks → NestJS](/docs/webhooks/nestjs) guide.
