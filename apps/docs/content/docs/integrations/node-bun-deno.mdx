...
description: Use MarzbanSDK in plain server runtimes — Node.js 19+, Bun 1.0+, and Deno 1.38+ — with no framework and zero extra configuration.
---

MarzbanSDK works out of the box in Node.js 19+, Bun 1.0+, and Deno 1.38+ with no additional configuration.
...
// Start log streaming — the returned handle isn't needed here since
// sdk.destroy() below closes every active stream at once
