...
export function useUsers() {
  ...
  async function fetchUsers(page = 0, limit = 50) {
    ...
    try {
      ...
      // sdk.user.* rejects with an SdkError subclass (HttpError, AuthError, ...)
      error.value = e instanceof SdkError ? e : null
    } finally {
      loading.value = false
    }
  }
...
import { useMarzban } from '../plugins/marzban'
import type { LogStreamHandle, WsError } from 'marzban-sdk'

export function useCoreLogs() {
  const sdk = useMarzban()
  const logs = ref<string[]>([])
  const error = ref<WsError | null>(null)
  let stream: LogStreamHandle | undefined

  async function start() {
    stream = await sdk.logs.connectByCore({
  ...
  return { logs, error, start, stop }
}
```

See [Reconnect & Stream Lifecycle](/docs/realtime/reconnect) for what `stream.state` reflects and how a drop is retried before `onError` ever fires.
