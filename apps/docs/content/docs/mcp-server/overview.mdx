...
- **A retried destructive call doesn't run twice** — an identical repeat within 5 minutes replays the first call's recorded result instead of executing again, and a call whose outcome was never observed says so instead of guessing. Every destructive result carries a field saying which of the two it is, so a replay can't be mistaken for a fresh run.
