...
2. Calls `marzban_system_get_stats` for panel-wide context — whether the core itself is running.
...
What it does:
1. Calls `marzban_system_get_stats` for panel-wide totals and current speed.
2. Calls `marzban_nodes_list` for per-node uplink/downlink over the period.
3. Calls `marzban_users_list`, then `marzban_users_get_usage` for the heaviest-looking users, to break their traffic down by node.
