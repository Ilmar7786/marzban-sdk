...
A token you *do* present is checked before that cache, not after it. So an explicit re-approval always registers as one: it runs the operation for real instead of being treated as a retry of the call it repeats (see [ADR-0020](https://github.com/Ilmar7786/marzban-sdk/blob/main/docs/adr/0020-confirm-auto-checks-a-token-before-the-trust-cache.md)). A retry that re-sends its already-spent token isn't turned into a new prompt by this — verification fails, and the call falls back to accumulated trust exactly as before.
