...
| `marzban_users_get_usage` | read | Traffic usage, with a per-node breakdown |
...
| Tool | Scope | Description |
|---|---|---|
| `marzban_system_get_stats` | read | Panel-wide CPU/memory/user-count/bandwidth stats, plus core version and running state |
| `marzban_system_get_inbounds` | read | List configured inbound proxies, grouped by protocol |
...
| Tool | Scope | Description |
|---|---|---|
| `marzban_subscription_get` | read | Look up subscription status/usage by the token from a subscription URL — no username needed |
