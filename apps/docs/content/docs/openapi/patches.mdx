...
Several upstream schemas are declared as `type: object` with no `properties` — proxy settings, parts of the core config. [kubb](https://kubb.dev) generates these as `z.object({})`, and Zod's default strip mode silently drops every key when parsing a real payload:
...
|---|---|
| `ProxySettings` | `user.proxies[protocol]` on every endpoint that returns a user — see the **Patched** column on the [OpenAPI Spec](/docs/openapi/spec) page |
| `CoreConfig` (get) | `GET /api/core/config` |
