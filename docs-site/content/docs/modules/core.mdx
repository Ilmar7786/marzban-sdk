...
Get runtime statistics for the Xray core process. Returns `CoreStats`.
...
Retrieve the full current Xray JSON configuration. Returns a dynamic `object` — the raw Xray config tree, passed through untyped.
...
Replace the Xray configuration with a new, dynamic `object` (the full Xray config tree). Triggers a core restart.
