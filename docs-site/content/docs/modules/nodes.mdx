...
List all configured nodes. Returns `NodeResponse[]`.
...
Get a single node by ID. Returns `NodeResponse`.
...
Register a new node. Accepts `NodeCreate`, returns `NodeResponse`.
...
Update a node's configuration. Accepts `NodeModify`, returns `NodeResponse`.
