...
Fetch the user's subscription config in the format for their VPN client. Returns `any` — the raw subscription payload (base64-encoded links or a client-specific config).
...
Get structured subscription metadata. Returns `SubscriptionUserResponse`.
