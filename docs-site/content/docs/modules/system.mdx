...
Get overall system resource and traffic statistics. Returns `SystemStats`.
...
List all available inbounds configured in Xray core. Returns `Record<string, ProxyInbound[]>` — inbound tags grouped by protocol.
...
Get the current proxy host configuration for all inbound tags. Returns `Record<string, ProxyHost[]>` — proxy hosts grouped by inbound tag.
