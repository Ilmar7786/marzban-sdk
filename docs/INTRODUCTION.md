...
- Get usage – `sdk.subscription.userGetUsage(token, params?)` (`params: { start?, end? }`)
...
This documentation covers **MarzbanSDK v3.x**.

### What's New in v3.0.0

- 🌐 Cross-runtime webhook signature verification via the Web Crypto API — no `node:crypto` is pulled into browser bundles
- 🔒 Webhook verification is now **server-side only**: it throws `WebhookEnvironmentError` if called in a browser
- ⚡ `parseWebhook` and `verifyWebhookSignature` are now **async** (return Promises)
- 📡 Native `WebSocket` is used when available (browser, Web Worker, Deno, Bun, Node.js 21+), with a lazy `ws` fallback for older Node

### Previously in v2.0.0
